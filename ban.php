...
print_r($result);
            ...
            $disp = mysqli_query($db, "SELECT * FROM channel WHERE c_id = '".$c_id."'");
      
            if (mysqli_num_rows($disp) > 0) {
                $read = mysqli_fetch_assoc($disp);
                }
               $sql_check = "SELECT * FROM ban WHERE c_id = '$c_id'";
            ...
            $rem = mysqli_query($db, "DELETE FROM channel WHERE c_id = '".$c_id."'");
            if($rem){
                       
                $bot->send_message($chatid, "✔️ Channel banned sucessfully", null, null, 'HTML');
            }}else{
                $bot->send_message($chatid, "❌ failed to ban channel.", null, null, 'HTML');
            }
        }else{
